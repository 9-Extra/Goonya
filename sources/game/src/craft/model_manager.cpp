...
BakedBlockModel ModelManager::bake_model(const BlockModel &model_src, TextureArrayAllocator &texture_allocator) {
    ...
    for (const auto &element : model_src.elements) {
        ...
        for (size_t i = 0; i < 6; ++i) {
            ...
            case Direction::DOWN: {
                quad.vertices[0] = {Vector3f{min.x, min.y, min.z}, Vector2f{uv0.x, uv1.y}};
                quad.vertices[1] = {Vector3f{max.x, min.y, min.z}, Vector2f{uv1.x, uv1.y}};
                quad.vertices[2] = {Vector3f{max.x, min.y, max.z}, Vector2f{uv1.x, uv0.y}};
                quad.vertices[3] = {Vector3f{min.x, min.y, max.z}, Vector2f{uv0.x, uv0.y}};
                is_cullface = min.y == 0;
                break;
            }
            case Direction::UP: {
                quad.vertices[0] = {Vector3f{min.x, max.y, min.z}, Vector2f{uv0.x, uv1.y}};
            ...
            case Direction::NORTH: {
                quad.vertices[0] = {Vector3f{min.x, min.y, min.z}, Vector2f{uv1.x, uv1.y}};
                quad.vertices[1] = {Vector3f{min.x, max.y, min.z}, Vector2f{uv1.x, uv0.y}};
                quad.vertices[2] = {Vector3f{max.x, max.y, min.z}, Vector2f{uv0.x, uv0.y}};
                quad.vertices[3] = {Vector3f{max.x, min.y, min.z}, Vector2f{uv0.x, uv1.y}};
                is_cullface = min.z == 0;
                break;
            }
            case Direction::SOUTH: {
                quad.vertices[0] = {Vector3f{min.x, min.y, max.z}, Vector2f{uv0.x, uv1.y}};
            ...
            case Direction::WEST: {
                quad.vertices[0] = {Vector3f{min.x, min.y, min.z}, Vector2f{uv1.x, uv1.y}};
                quad.vertices[1] = {Vector3f{min.x, min.y, max.z}, Vector2f{uv0.x, uv1.y}};
                quad.vertices[2] = {Vector3f{min.x, max.y, max.z}, Vector2f{uv0.x, uv0.y}};
                quad.vertices[3] = {Vector3f{min.x, max.y, min.z}, Vector2f{uv1.x, uv0.y}};
                is_cullface = min.x == 0;
                break;
            }
            case Direction::EAST: {
                quad.vertices[0] = {Vector3f{max.x, min.y, min.z}, Vector2f{uv0.x, uv1.y}};
...
private:
    ...
    const Json::Value* fetch_model_json(const std::string& location){ 
        if (auto iter = json_model_cache.find(location);iter != json_model_cache.end()){
            ...
            std::filesystem::path path = resource_path / std::format("{}/models/{}.json", parsed_location.name_space, parsed_location.key);
...
static std::vector<std::tuple<BlockStateProperty*, BlockStatePropertyValue>> parse_state_variant_key(const std::string& variant_key){
...
static bool is_match_block_state(const std::vector<std::tuple<BlockStateProperty*, BlockStatePropertyValue>>& property_pairs, BlockState* state){
    for(const auto& [p, v]: property_pairs){
        // 假定属性一定存在
        if (state->get_property_raw_value(p) != v){
            return false;
        }
    }
    return true;
...
void ModelManager::load_all_models() {
    ...
    for(const auto& [block, _]: REGISTRY_BLOCK){
        ...
        std::filesystem::path path = resource_path / std::format("{}/blockstates/{}.json", location.name_space, location.key);
        std::ifstream file(path, std::ios::binary | std::ios::in);
        if (!file) {
        ...
        if (blockstate_json.isMember("variants")){
            ...
            for(const Json::String& variant_key: blockstate_json["variants"].getMemberNames()){
                const std::vector<std::tuple<BlockStateProperty*, BlockStatePropertyValue>> property_pairs = parse_state_variant_key(variant_key);
                for(const std::unique_ptr<BlockState>& _state: states){
                    ...
                    if (is_match_block_state(property_pairs, state)){
                        if (blockstate_model_map.contains(state)){
                            LOG_ERROR("Exception loading blockstate definition: '{}' for variant: '{}'", location, variant_key);
                            continue;
                        }
                        const Json::Value& variant_content_list = blockstate_json["variants"][variant_key];
                        // 对于多选一的模型，目前固定选择第一个
                        const Json::Value& variant_content = variant_content_list.isArray() ? variant_content_list[0] : variant_content_list;
