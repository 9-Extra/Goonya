...
public:
    ...
    uint32_t entry_count() const noexcept { return (uint32_t)entry_list.size(); }
    bool contains(T *entry) const noexcept { return entry_to_id.contains(entry); }
    bool contains(std::string_view key) const noexcept { return key_to_id.contains(key); }
    ...
    T *find_entry(TID id) const noexcept { return id < entry_list.size() ? entry_list[id].get() : nullptr; };
    ...
    decltype(auto) begin() const noexcept { return entry_to_id.begin(); }
    decltype(auto) end() const noexcept { return entry_to_id.end(); }
};
