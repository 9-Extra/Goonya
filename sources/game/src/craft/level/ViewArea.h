...
public:
    explicit ViewArea(uint32_t view_distance, Vector3i center) : origin(center) { set_view_distance(view_distance); }
    ~ViewArea() = default;
    ...
    uint32_t get_view_distance() const noexcept { return view_distance; }

    const Vector3i &get_origin() const noexcept { return origin; }
