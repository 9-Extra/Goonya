...
public:
    ...
    void *map(BufferMapOption option) const noexcept override { return map_range(option, 0, this->size); };
    void *map_range(BufferMapOption option, size_t offset, size_t size) const noexcept override {
        if (size == 0) {
            return nullptr;
        }
        GLenum access = 0;
