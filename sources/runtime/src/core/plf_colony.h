...
#define PLF_NOEXCEPT_ALLOCATOR noexcept(noexcept(allocator_type()))
#endif
#elif defined(__GLIBCXX__) // Using another compiler type with libstdc++ - we are assuming full c++11 compliance for
                           // compiler - which may not be true
...
// Until such point as standard libraries include std::ranges::from_range_t, including this so the rangesv3 constructor
// overloads will work unambiguously:
namespace ranges {
struct from_range_t {};
...
struct less {
    bool operator()(const element_type &a, const element_type &b) const noexcept { return a < b; }
};
...
struct equal_to {
    ...
    explicit equal_to(const element_type &store_value) noexcept : value(store_value) {}

    bool operator()(const element_type &compare_value) const noexcept { return value == compare_value; }
};

// To enable conversion to void * when allocator supplies non-raw pointers:
template <class source_pointer_type>
static PLF_CONSTFUNC void *void_cast(const source_pointer_type source_pointer) noexcept {
...
struct limits {
    size_t min, max;
    PLF_CONSTFUNC limits(const size_t minimum, const size_t maximum) noexcept : min(minimum), max(maximum) {}
};
...
class colony : private allocator_type // Empty base class optimisation - inheriting allocator functions
{
    ...
    typedef typename plf::conditional<(priority == performance &&
        ...
        skipfield_type; // Note: unsigned short is equivalent to uint_least16_t ie. Using 16-bit unsigned integer in
                        // best-case scenario, greater-than-16-bit unsigned integer where platform doesn't support
                        // 16-bit types. unsigned char is always == 1 byte, as opposed to uint_8, which may not be.
...
public:
    ...
    // The element as allocated in memory needs to be at-least 2*skipfield_type width in order to support free list
    // indexes in erased element memory space, so: make the size of this struct the larger of alignof(T), sizeof(T) or
    // 2*skipfield_type (the latter is only relevant for type char/uchar), and make the alignment alignof(T). This type
    // is used mainly for correct pointer arithmetic while iterating over elements in memory.
    struct alignas(alignof(element_type)) aligned_element_struct {
        // Using char as sizeof is always guaranteed to be 1 byte regardless of the number of bits in a byte on given
        // computer, whereas for example, uint8_t would fail on machines where there are more than 8 bits in a byte eg.
        // Texas Instruments C54x DSPs.
        char
            data[(sizeof(element_type) < (sizeof(skipfield_type) * 2))
                     ? ((sizeof(skipfield_type) * 2) < alignof(element_type) ? alignof(element_type)
    ...
    // allocated as an aligned type with the same alignment as T in order for the elements to align with memory
    // boundaries correctly (which won't happen if we allocate as char or uint_8). But the larger the sizeof in the type
    // we use for allocation, the greater the chance of creating a lot of unused memory in the skipfield portion of the
    // allocated block. So we create a type that is sizeof(alignof(T)), as in most cases alignof(T) < sizeof(T). If
...
private:
    // Calculate the capacity of a group's elements+skipfield memory block when expressed in multiples of the
    // value_type's alignment (rounding up).
    ...
    static PLF_CONSTFUNC destination_pointer_type pointer_cast(const source_pointer_type source_pointer) noexcept {
        // constexpr necessary to avoid a branch for every call
    ...
    typedef typename std::allocator_traits<allocator_type>::template rebind_alloc<aligned_element_struct>
    ...
    struct group {
        skipfield_pointer_type
            skipfield; // Skipfield storage. The element and skipfield arrays are allocated contiguously, in a single
                       // allocation, in this implementation, hence the skipfield pointer also functions as a
                       // 'one-past-end' pointer for the elements array. There will always be one additional skipfield
                       // node allocated compared to the number of elements. This is to ensure a faster ++ iterator
                       // operation (fewer checks are required when this is present). The extra node is unused and
                       // always zero, but checked, and not having it will result in out-of-bounds memory errors. This
                       // is present before elements in the group struct as it is referenced constantly by the ++
                       // operator, hence having it first results in a minor performance increase.
        group_pointer_type next_group; // Next group in the linked list of all groups. NULL if no following group. 2nd
                                       // in struct because it is so frequently used during iteration.
        ...
        skipfield_type free_list_head; // The index of the last erased element in the group. The last erased element
                                       // will, in turn, contain the number of the index of the next erased element, and
                                       // so on. If this is == maximum skipfield_type value then free_list is empty ie.
                                       // no erasures have occurred in the group (or if they have, the erased locations
                                       // have subsequently been reused via insert/emplace/assign).
        const skipfield_type capacity; // The element capacity of this particular group - can also be calculated from
        ...
        skipfield_type size; // The total number of active elements in group - changes with insert and erase commands -
        ...
        group_pointer_type erasures_list_next_group,
            erasures_list_previous_group; // The next and previous groups in the list of groups with erasures ie. with
                                          // active erased-element free lists. NULL if no next or previous group.
        size_type group_number; // Used for comparison (> < >= <= <=>) iterator operators (used by distance function and
                                // user).
        ...
        group(aligned_struct_allocator_type &aligned_struct_allocator, const skipfield_type elements_per_group,
            ...
            : next_group(NULL),
              elements(pointer_cast<aligned_pointer_type>(PLF_ALLOCATE(
                  aligned_struct_allocator_type, aligned_struct_allocator,
                  get_aligned_block_capacity(elements_per_group), (previous == NULL) ? NULL : previous->elements))),
              previous_group(previous), free_list_head(std::numeric_limits<skipfield_type>::max()),
              capacity(elements_per_group), size(1), erasures_list_next_group(NULL), erasures_list_previous_group(NULL),
        ...
        void reset(const skipfield_type increment, const group_pointer_type next, const group_pointer_type previous,
            ...
            std::memset(
                ...
                sizeof(skipfield_type) *
                    static_cast<size_type>(capacity)); // capacity + 1 is not necessary here as the final skipfield node
    ...
    group_pointer_type erasure_groups_head, // Head of doubly-linked list of groups which have erased-element memory
                                            // locations available for re-use
        unused_groups_head; // Head of singly-linked list of reserved groups retained by erase()/clear() or created by
                            // reserve()
    ...
    void blank() noexcept {
        if constexpr (std::is_standard_layout<colony>::value &&
                      ...
                      std::is_trivially_destructible<skipfield_pointer_type>::
                          value) { // If all pointer types are trivial, we can just nuke the member variables from orbit
                                   // with memset (NULL is always 0):
    ...
    static PLF_CONSTFUNC size_t max_size_static() noexcept {
...
public:
    ...
    static PLF_CONSTFUNC skipfield_type block_capacity_default_min() noexcept {
        ...
        const skipfield_type max_block_capacity =
            block_capacity_default_max(); // Necessary to check against in situations with > 64bit pointer sizes and
                                          // small sizeof(T)
        return std::max(static_cast<skipfield_type>(8), std::min(adaptive_size, max_block_capacity));
    }

    // Adaptive maximum based on numeric_limits and best outcome from multiple benchmark's (on balance) in terms of
    // memory usage and performance:
    static PLF_CONSTFUNC skipfield_type block_capacity_default_max() noexcept {
    ...
    static PLF_CONSTFUNC plf::limits block_capacity_default_limits() noexcept {
    ...
    PLF_CONSTFUNC explicit colony(const allocator_type &alloc) noexcept
        : allocator_type(alloc), erasure_groups_head(NULL), unused_groups_head(NULL), total_size(0), total_capacity(0),
          min_block_capacity(block_capacity_default_min()), max_block_capacity(block_capacity_default_max()),
          group_allocator(*this), aligned_struct_allocator(*this), skipfield_allocator(*this), tuple_allocator(*this) {}

    PLF_CONSTFUNC colony() PLF_NOEXCEPT_ALLOCATOR : erasure_groups_head(NULL),
                                                    unused_groups_head(NULL),
                                                    total_size(0),
                                                    total_capacity(0),
    ...
    PLF_CONSTFUNC colony(const plf::limits block_limits, const allocator_type &alloc) noexcept
        : allocator_type(alloc), erasure_groups_head(NULL), unused_groups_head(NULL), total_size(0), total_capacity(0),
              ...
              std::max(source.min_block_capacity,
                       static_cast<skipfield_type>(std::min(
                           ...
                           static_cast<size_type>(
                               source.max_block_capacity))))), // min group size is set to value closest to total number
                                                               // of elements in source colony, in order to not create
                                                               // unnecessary small groups in the range-insert below,
                                                               // then reverts to the original min group size
                                                               // afterwards. This effectively saves a call to reserve.
          max_block_capacity(source.max_block_capacity), group_allocator(*this), aligned_struct_allocator(*this),
          skipfield_allocator(*this),
          tuple_allocator(*this) { // can skip checking for skipfield conformance here as source will have already
    ...
    colony(const colony &source)
        :
          ...
          erasure_groups_head(NULL), unused_groups_head(NULL), total_size(0), total_capacity(0),
          min_block_capacity(std::max(source.min_block_capacity,
                                      static_cast<skipfield_type>(std::min(
    ...
    colony(colony &&source, const std::type_identity_t<allocator_type> &alloc)
        : allocator_type(alloc), end_iterator(source.end_iterator), begin_iterator(source.begin_iterator),
          erasure_groups_head(source.erasure_groups_head), unused_groups_head(source.unused_groups_head),
          total_size(source.total_size), total_capacity(source.total_capacity),
          min_block_capacity(source.min_block_capacity), max_block_capacity(source.max_block_capacity),
          group_allocator(alloc), aligned_struct_allocator(alloc), skipfield_allocator(alloc), tuple_allocator(alloc) {
        assert(&source != this);
    ...
    colony(colony &&source) noexcept
        : allocator_type(static_cast<allocator_type &>(source)), end_iterator(std::move(source.end_iterator)),
          begin_iterator(std::move(source.begin_iterator)), erasure_groups_head(std::move(source.erasure_groups_head)),
          unused_groups_head(std::move(source.unused_groups_head)), total_size(source.total_size),
    ...
    reverse_iterator rbegin() noexcept {
        return (end_iterator.group_pointer != NULL)
                   ...
                   : reverse_iterator(begin_iterator.group_pointer, begin_iterator.element_pointer - 1,
    ...
    const_reverse_iterator crbegin() const noexcept {
        return (end_iterator.group_pointer != NULL)
                   ? ++const_reverse_iterator(end_iterator.group_pointer, end_iterator.element_pointer,
                                              end_iterator.skipfield_pointer)
                   : const_reverse_iterator(begin_iterator.group_pointer, begin_iterator.element_pointer - 1,
                                            begin_iterator.skipfield_pointer - 1);
    }
    ...
    ~colony() noexcept { destroy_all_data(); }
...
private:
    ...
    PLF_CONSTFUNC void destroy_element(const aligned_pointer_type element) noexcept {
        // To avoid codegen for this function with trivially-destructible types. CPP20 because we don't want to trigger
        // a branch for every destruction
            ...
            if constexpr (!std::is_trivially_destructible<element_type>::value) {
                if (total_size != 0) {
                    while (begin_iterator.group_pointer !=
                           end_iterator
                               .group_pointer) // Erase elements without bothering to update skipfield - much faster:
                    {
    ...
    void edit_free_list_prev(const aligned_pointer_type location,
                             const skipfield_type value) noexcept // Write to the 'previous erased element' index in the
    ...
    void update_skipblock(const iterator &new_location, const skipfield_type prev_free_list_index) noexcept {
        ...
        } else // single-node skipblock, remove skipblock
        {
            ...
            {
                erasure_groups_head =
                    erasure_groups_head->erasures_list_next_group; // No need to update previous group for new head, as
                                                                   // this is never accessed if group == head
            }
        }
        ...
        *(new_location.skipfield_pointer) = 0;
        ...
        if (new_location.group_pointer == begin_iterator.group_pointer &&
            new_location.element_pointer <
                begin_iterator.element_pointer) { /* ie. begin_iterator was moved forwards as the result of an erasure
                                                     at some point, this erased element is before the current begin,
    ...
    void reset_group_numbers() noexcept { update_subsequent_group_numbers(0, begin_iterator.group_pointer); }
...
public:
    ...
    iterator insert(const element_type &element) {
        if (end_iterator.element_pointer != NULL) {
            ...
            } else // there are erased elements, reuse those memory locations
            {
                ...
                // information for that skipblock is stored. Get the previous free-list node's index from this memory
                // space, before we write to our element to it. 'Next' index is always the free_list_head (as
                // represented by the maximum value of the skipfield type) here so we don't need to get it:
    ...
    iterator
    insert([[maybe_unused]] const_iterator hint,
    ...
    iterator
    insert(element_type &&element) // The move-insert function is near-identical to the regular insert function, with
    ...
    iterator emplace(
        arguments &&...parameters) // The emplace function is near-identical to the regular insert function, with the
...
private:
    ...
    void recover_from_partial_fill() {
        if constexpr ((!std::is_copy_constructible<element_type>::value &&
                      ...
                      !std::is_nothrow_copy_constructible<element_type>::value) // to avoid unnecessary codegen, since
                                                                                // this function will never be called if
                                                                                // this line isn't true
        {
        ...
        if constexpr (std::is_nothrow_copy_constructible<element_type>::value) {
            if constexpr (sizeof(aligned_element_struct) != sizeof(element_type)) {
                alignas(alignof(aligned_element_struct)) element_type aligned_copy =
                    element; // to avoid potentially violating memory boundaries in line below, create an initial object
                             // copy of same (but aligned) type
    ...
    void recover_from_partial_skipblock_fill(const aligned_pointer_type location,
        ...
        if constexpr ((!std::is_copy_constructible<element_type>::value &&
                       !std::is_nothrow_move_constructible<element_type>::value) ||
                      !std::is_nothrow_copy_constructible<element_type>::value) // to avoid unnecessary codegen

        {
            ...
            erasure_groups_head->size =
                static_cast<skipfield_type>(erasure_groups_head->size + elements_constructed_before_exception);
            total_size += elements_constructed_before_exception;
    ...
    void fill_unused_groups(size_type size, const element_type &element, size_type group_number,
        ...
        for (end_iterator.group_pointer = current_group; end_iterator.group_pointer->capacity < size;
            ...
            end_iterator.group_pointer->reset(capacity, end_iterator.group_pointer->next_group, previous_group,
                                              group_number++);
...
public:
    ...
    void insert(size_type size, const element_type &element) {
        ...
        while (erasure_groups_head !=
               NULL) // skipblock loop: breaks when colony is exhausted of reusable skipblocks, or returns if size == 0
        {
            ...
            if (skipblock_size <= size) {
                ...
                if (erasure_groups_head->free_list_head !=
                    std::numeric_limits<skipfield_type>::max()) // ie. there are more skipblocks to be filled in this
                                                                // group
                {
                    edit_free_list_next(erasure_groups_head->elements + erasure_groups_head->free_list_head,
                                        std::numeric_limits<skipfield_type>::max()); // set 'next' index of new free
                                                                                     // list head to 'end' (numeric max)
                } else {
        ...
        // This variable is either the remaining capacity of the group or the number of elements yet to be inserted,
        // whichever is smaller:
        const skipfield_type group_remainder = static_cast<skipfield_type>(
            std::min(static_cast<size_type>(pointer_cast<aligned_pointer_type>(end_iterator.group_pointer->skipfield) -
                                            end_iterator.element_pointer),
                     size));
        ...
        fill_unused_groups(size, element, end_iterator.group_pointer->group_number + 1u, end_iterator.group_pointer,
                           unused_groups_head);
    }
...
private:
    ...
    void range_fill(iterator_type &it, const skipfield_type size) {
        ...
        } else if constexpr (std::is_nothrow_move_constructible<element_type>::value &&
    ...
    void range_insert(
        iterator_type it,
        size_type size) // this is near-identical to the fill insert, with the only alteration being incrementing an
                        // iterator for construction, rather than using a const element. And the fill etc function calls
                        // are changed to range_fill to match this pattern. See fill insert for code explanations
    {
        if (size == 0) {
            return;
        } else if (size == 1) {
        ...
        range_fill_unused_groups(size, it, end_iterator.group_pointer->group_number + 1u, end_iterator.group_pointer,
                                 unused_groups_head);
    }
...
public:
    ...
    template <class iterator_type>
    void
    insert(const typename plf::enable_if<!std::numeric_limits<iterator_type>::is_integer, iterator_type>::type first,
...
private:
    ...
    void add_group_to_unused_groups_list(const group_pointer_type group_pointer) noexcept {
...
public:
    ...
    erase(const const_iterator it) // if uninitialized/invalid iterator supplied, function could generate an exception
    {
        ...
        if (--(it.group_pointer->size) != 0) // ie. non-empty group at this point in time, don't consolidate
        {
            ...
            // If node only has skipblock on left, set current node and start node of the skipblock to left node value
            // + 1. If node only has skipblock on right, make this node the start node of the skipblock and update end
            // node If node has skipblocks on left and right, set start node of left skipblock and end node of right
            // skipblock to the values of the left + right nodes + 1

            // Optimization explanation:
            // The contextual logic below is the same as that in the insert() functions but in this case the value of
            // the current skipfield node will always be zero (since it is not yet erased), meaning no additional
            // manipulations are necessary for the previous skipfield node comparison - we only have to check against
            // zero
            ...
            const char after_skipfield = *(it.skipfield_pointer + 1) !=
                                         0; // NOTE: boundary test (checking against end-of-elements) is able to be
                                            // skipped due to the extra skipfield node (compared to element field) -
                                            // which is present to enable faster iterator operator ++ operations
            ...
            } else // both preceding and following consecutive erased elements - erased element is between two
                   // skipblocks
            {
                *(it.skipfield_pointer) = 1; // This line necessary in order for get_iterator() to work - ensures that
            ...
            if (return_iterator.element_pointer == pointer_cast<aligned_pointer_type>(it.group_pointer->skipfield) &&
            ...
            if (it.element_pointer == begin_iterator.element_pointer)
                begin_iterator = return_iterator; // If original iterator was first element in colony, update it's value
                                                  // with the next non-erased element:
        ...
        } else if ((!in_back_block) & in_front_block) // ie. Remove first group, change first group to next group
        {
            ...
            if (it.group_pointer->free_list_head !=
                std::numeric_limits<skipfield_type>::max()) // Erasures present within the group, ie. was part of the
            ...
            // note: end iterator only needs to be changed if the deleted group was the final group in the chain ie. not
            // in this case
            begin_iterator.element_pointer =
                begin_iterator.group_pointer->elements +
                *(begin_iterator.group_pointer->skipfield); // If the beginning index has been erased (ie. skipfield !=
                                                            // 0), skip to next non-erased element
            ...
            return begin_iterator;
                     ...
                     in_front_block)) // this is a non-first group but not final group in chain: delete the group, then
                                      // link previous group to the next group in the chain:
        {
            ...
            if constexpr (priority == performance) {
        ...
        } else // this is a non-first group and the final group in the chain
        {
            ...
            end_iterator.group_pointer =
                it.group_pointer->previous_group; // end iterator needs to be changed as element supplied was the back
            ...
            if constexpr (priority == performance) {
                ...
                if (unused_groups_head !=
                    NULL) // priority == memory_use, if there are other reserved blocks already, get rid of this one
                {
                    total_capacity -= it.group_pointer->capacity;
                    deallocate_group(it.group_pointer);
    ...
    iterator
    erase(const const_iterator iterator1,
                                          ...
                                          // exception. If iterator1 > iterator2, behaviour is undefined.
    {
        ...
        if (current.group_pointer != iterator2.group_pointer) // ie. if start and end iterators are in separate groups
        {
            if (current.element_pointer !=
                current.group_pointer->elements +
                    *(current.group_pointer->skipfield)) // if iterator1 is not the first non-erased element in it's
                                                         // group - most common case
            {
                ...
                // Schema: first erase all non-erased elements until end of group & remove all skipblocks post-iterator1
                // from the free_list. Then, either update preceding skipblock or create new one:
                        ...
                        } else // remove skipblock from group:
                        {
                            ...
                            if (next_free_list_index == std::numeric_limits<skipfield_type>::max() &&
                                prev_free_list_index ==
                                    std::numeric_limits<skipfield_type>::max()) // if this is the last skipblock in the
                                                                                // free list
                            {
                                remove_from_groups_with_erasures_list(
                                    iterator1.group_pointer); // remove group from list of free-list groups - will be
                                ...
                                if constexpr (!std::is_trivially_destructible<element_type>::value) {
                                    while (current.element_pointer !=
                                           end) // miniloop - avoid checking skipfield for rest of elements in group, as
                                                                                   ...
                                                                                   // list
                            {
                                ...
                                edit_free_list_next(current.group_pointer->elements + prev_free_list_index,
                                                    std::numeric_limits<skipfield_type>::max());
                            } else // either a tail or middle free list node
                            {
                                edit_free_list_prev(current.group_pointer->elements + next_free_list_index,
                                                    prev_free_list_index);
                ...
                const skipfield_type previous_node_value =
                    *(iterator1.skipfield_pointer -
                      1); // safe to do this here as we've already established that we're not at start of skipfield
                ...
                if (previous_node_value == 0) // no previous skipblock
                {
                    ...
                    *(iterator1.skipfield_pointer + distance_to_end - 1) = distance_to_end; // set end node value
                    ...
                    const skipfield_type index =
                        static_cast<skipfield_type>(iterator1.element_pointer - iterator1.group_pointer->elements);

                    if (iterator1.group_pointer->free_list_head !=
                        std::numeric_limits<skipfield_type>::max()) // ie. if this group already has some erased
                                                                    // elements
                    {
                        edit_free_list_next(iterator1.group_pointer->elements + iterator1.group_pointer->free_list_head,
                                            index); // set prev free list head's 'next index' number to the index of the
                                                    // iterator1 element
                    } else {
                ...
                if (distance_to_end > 2) // if the skipblock is longer than 2 nodes, fill in the middle nodes with
            ...
            if (previous_group != NULL) {
                ...
                ; // This line is included here primarily to avoid a secondary if statement within the if block below -
                  // it will not be needed in any other situation
            }
        }
        ...
        // destructor), 2. if no elements left in colony, reset the group 3. otherwise reset end_iterator and remove
        // group from groups-with-erasures list (if free list of erasures present)

        if (current.element_pointer != iterator2.element_pointer) // in case iterator2 was at beginning of it's group -
                ...
                current.element_pointer !=
                    current.group_pointer->elements +
                        *(current.group_pointer
                              ->skipfield)) // ie. not erasing entire group. Note: logistically the only way the entire
                                            // group can be erased here is if iterator2 == end - otherwise would be
                                            // caught by the if block above. Second condition in this if statement only
                                            // possibly applies if iterator1.group_pointer == iterator2.group_pointer
            {
                size_type number_of_group_erasures = 0;
                    ...
                    *(iterator2.skipfield_pointer - 1) = distance_to_iterator2;
                    ...
                    if (iterator2.group_pointer->free_list_head !=
                        std::numeric_limits<skipfield_type>::max()) // ie. if this group already has some erased
                                                                    // elements
                    {
                        edit_free_list_next(iterator2.group_pointer->elements + iterator2.group_pointer->free_list_head,
                                            index);
                    } else {
                ...
                if (distance_to_iterator2 > 2) // if the skipblock is longer than 2 nodes, fill in the middle nodes with
                                               // non-zero values so that get_iterator() and is_active() will work
                {
            ...
            } else // ie. full group erasure
            {
                    ...
                    0) // ie. previous_group != NULL - colony is not empty
                {
                    ...
                    if constexpr (priority == performance) {
                        ...
                        if (unused_groups_head != NULL) // priority == memory_use, if there are other reserved blocks
                                                        // already, get rid of this one
                        {
                            total_capacity -= current.group_pointer->capacity;
                            deallocate_group(current.group_pointer);
                        } else // otherwise, retain - prevents unnecessary allocations/deallocations with stack-like
                               // usage
                        {
                            add_group_to_unused_groups_list(current.group_pointer);
                        }
                ...
                {
                    if constexpr (priority == memory_use) {
                        trim_capacity();
                    }
...
public:
    ...
    void assign(size_type size, const element_type &element) {
        ...
        if constexpr ((std::is_trivially_destructible<element_type>::value &&
                      ...
                      !std::is_copy_assignable<element_type>::value) // ie. If there is no benefit nor difference to
                                                                     // assigning vs constructing, or if we can't
                                                                     // assign, use faster method:
        {
            prepare_groups_for_assign(size);
            fill_unused_groups(size, element, 0, NULL, begin_iterator.group_pointer);
        } else
...
public:
    ...
    template <class iterator_type>
    void
    assign(const typename plf::enable_if<!std::numeric_limits<iterator_type>::is_integer, iterator_type>::type &first,
           const iterator_type &last) {
        range_assign(first, static_cast<size_type>(std::distance(first, last)));
    }

    // Overloads for colony iterators, since std::distance overloads for these are not possible without C++20 concepts
    // and we must stick with ADL:
    template <class iterator_type>
    ...
    size_type max_size() const noexcept { return std::allocator_traits<allocator_type>::max_size(*this); }
    ...
    size_type memory() const noexcept {
        ...
        end_iterator.group_pointer->next_group =
            unused_groups_head; // temporarily link the main groups and unused groups (reserved groups) in order to only
                                // have one loop below instead of several
        ...
        for (group_pointer_type current = begin_iterator.group_pointer; current != NULL;
            ...
            memory_use +=
                ...
                (get_aligned_block_capacity(current->capacity) *
                 sizeof(
                     aligned_allocation_struct)); // add memory block sizes and the size of the group structs
                                                  // themselves. The original calculation, including divisor, is
                                                  // necessary in order to correctly round up the number of allocations
        }
    ...
    static PLF_CONSTFUNC size_type block_metadata_memory(const size_type block_capacity) noexcept {
        return sizeof(group) +
               ((get_aligned_block_capacity(static_cast<skipfield_type>(block_capacity)) - block_capacity) *
                sizeof(aligned_allocation_struct));
    }

    static PLF_CONSTFUNC size_type block_allocation_amount(size_type block_capacity) noexcept {
    ...
    static PLF_CONSTFUNC size_type max_elements_per_allocation(const size_type allocation_amount) noexcept {
        ...
        // of the element type's alignof:
        if ((/* Explanation: elements and skipfield are allocated in a single allocation to save performance.
                     In order for the elements to be correctly aligned in memory, this single allocation is aligned to
                the alignof the element type, so the first line below is the allocation amount in bytes required for the
                skipfield when allocated in multiples of the element type's alignof. The + sizeof(skipfield_type) adds
                the additional skipfield node as mentioned, and the (num_units + 1) minus 1 byte rounds up the integer
                division: */
             (((((num_units + 1) * sizeof(aligned_allocation_struct)) - 1) + sizeof(skipfield_type)) /
              sizeof(aligned_allocation_struct))
            ...
            > allocation_amount) {
            --num_units; // In this implementation it is not possible for the necessary adjustment to be greater than 1
    ...
    size_type group_size_sum() const noexcept {
        size_type temp = 0;
...
private:
    // get all elements contiguous in memory and shrink to fit, remove erasures and free lists. Invalidates all
    // iterators and pointers to elements.
    void consolidate(const skipfield_type new_min, const skipfield_type new_max) {
        ...
        if constexpr (!std::is_trivially_copyable<element_type>::value &&
                      std::is_nothrow_move_constructible<element_type>::value) {
            temp.range_fill_unused_groups(total_size, plf::make_move_iterator(begin_iterator), 0, NULL,
...
public:
        ...
        if (total_capacity != 0) {
            if (total_size != 0) {
                if (min_block_capacity > new_max ||
                    max_block_capacity < new_min) // If none of the original blocks could potentially fit within the new
                                                  // limits, skip checking of blocks and just consolidate:
                {
                ...
                {
                    // Otherwise need to check all group sizes here (not just back one, which is most likely largest),
                    // because splice might append smaller blocks after a larger block:
                    for (group_pointer_type current_group = begin_iterator.group_pointer; current_group != NULL;
            ...
            // If a consolidation or throw has not occured, process reserved/unused groups and deallocate where they
            // don't fit the new limits:
    ...
    PLF_CONSTFUNC plf::limits block_capacity_limits() const noexcept {
        return plf::limits(static_cast<size_t>(min_block_capacity), static_cast<size_t>(max_block_capacity));
    }

    static PLF_CONSTFUNC plf::limits block_capacity_hard_limits() noexcept {
        return plf::limits(
...
private:
    void move_assign(colony &&source) noexcept {
...
public:
    // Move assignment
    colony &operator=(colony &&source) noexcept(
        ...
        if constexpr (std::allocator_traits<allocator_type>::propagate_on_container_move_assignment::value ||
            ...
            move_assign(std::move(source));
        } else // Allocator isn't movable so move elements from source and deallocate the source's blocks. Could throw
               // here:
        {
    ...
    friend bool operator!=(const colony &lh, const colony &rh) noexcept { return !(lh == rh); }
    ...
    void trim_capacity(const size_type capacity_retain) noexcept {
        ...
        if (begin_iterator.element_pointer == end_iterator.element_pointer) // ie. clear() has been called prior
        {
            if (number_of_elements_to_remove >= begin_iterator.group_pointer->capacity) {
                ...
                if (unused_groups_head != NULL) // some of the reserved blocks were not removed as they were too large,
                                                // so use one of these to make the new begin group
                {
                    begin_iterator.group_pointer = unused_groups_head;
    ...
    void reserve(size_type new_capacity) {
        ...
        skipfield_type remainder =
                           static_cast<skipfield_type>(new_capacity - (number_of_max_groups * max_block_capacity)),
        ...
        if (remainder == 0) {
            ...
            if (unused_groups_head != NULL && max_block_capacity - remainder >= min_block_capacity) {
                // If there exists an unused group which's of low-enough capacity, deallocate that and add it's capacity
                // to the remainder group
            ...
            if (remainder < min_block_capacity) // Implies we were unable to consolidate remainder with an existing
                                                // unused group, in the if-block above
            {
                // Note: negative_remainder is used to take the difference between the minimum block capacity limit and
                // the actual remainder, and spread this negative difference over subsequent blocks which are in the
                // usual case at max capacity.
                negative_remainder = min_block_capacity - remainder;
                remainder = min_block_capacity;

                // This line checks to see, if we have to reduce the size of the max-capacity blocks to spread the
                // negative_remainder out, whether even reducing the max blocks to min capacity will be enough to keep
                // the capacity under max_size(). We add 1 for the initial (remainder) block. This guards against
                // situations where, for example, the min/max limits are very similar so spreading the negative
                // remainder out is less doable:
                if (total_capacity + ((number_of_max_groups + 1) * min_block_capacity) > max_size()) {

                    throw std::length_error("Reserve cannot increase capacity to >= n without being > max_size() due "
...
private:
    template <bool is_const>
    colony_iterator<is_const> get_it(const pointer element_pointer) const noexcept {
        if (end_iterator.group_pointer != NULL) {
            ...
            // Note: we start with checking the back group first, as it will be the largest group in most cases, so
            // there's a statistically-higher chance of the element being within it.

            // Special case for back group in case the element was in a group which became empty and got moved to the
            // unused_groups list or was deallocated, and then that memory was re-used (ie. it became the current back
            // group). The following prevents the function from mistakenly returning an iterator which is beyond the
            // back element of the colony:
            if (aligned_element_pointer >= end_iterator.group_pointer->elements &&
                ...
                return (*skipfield_pointer == 0) ? colony_iterator<is_const>(end_iterator.group_pointer,
            ...
            for (group_pointer_type current_group = end_iterator.group_pointer->previous_group; current_group != NULL;
                ...
                if (aligned_element_pointer >= current_group->elements &&
                    aligned_element_pointer < pointer_cast<aligned_pointer_type>(current_group->skipfield)) {
                    const skipfield_pointer_type skipfield_pointer =
                        current_group->skipfield + (aligned_element_pointer - current_group->elements);
...
public:
    iterator get_iterator(const pointer element_pointer) noexcept { return get_it<false>(element_pointer); }
    ...
    bool is_active(const const_iterator &it) const noexcept {
        if (end_iterator.group_pointer != NULL) {
            // Schema: check (a) that the group the iterator belongs to is still active and not deallocated or in the
            // unused_groups list, then (b) that the element is not erased. (a) prevents an out-of-bounds memory access
            // if the group is deallocated. Same reasoning as get_iterator for loop conditions
            ...
            for (group_pointer_type current_group = end_iterator.group_pointer->previous_group; current_group != NULL;
                 current_group = current_group->previous_group) {
                if (it.group_pointer == current_group && it.element_pointer >= current_group->elements &&
                    it.element_pointer <
                        pointer_cast<aligned_pointer_type>(
                            current_group
                                ->skipfield)) // 2nd 2 conditions necessary in case the group contained the element
                                              // which the iterator points to, has been deallocated from the colony
                                              // previously, but then the same pointer address is re-supplied via an
                                              // allocator for a subsequent group allocation (in which case the group's
    ...
    allocator_type get_allocator() const noexcept { return static_cast<allocator_type>(*this); }

private:
    void source_blocks_incompatible() {
        throw std::length_error("A source memory block capacity is outside of the destination's minimum or maximum "
...
public:
    void splice(colony &source) {
        ...
        // If the source has more unused memory spaces in the back group than the destination, swap them before
        // processing to reduce the number of locations added to a free list and also subsequent jumps during iteration.
        ...
        if (source.min_block_capacity > max_block_capacity ||
            source.max_block_capacity <
                min_block_capacity) // ie. source blocks cannot possibly fit within *this's block capacity limits
        {
        ...
        if (total_size != 0) {
            ...
            // source, swap with source to reduce number of skipped elements during iteration:
            if ((pointer_cast<aligned_pointer_type>(end_iterator.group_pointer->skipfield) -
                ...
                swap(source);
                // Swap back unused groups list and block capacity limits so that source and *this retain their original
                // ones:
            ...
            const skipfield_type distance_to_end =
                static_cast<skipfield_type>(pointer_cast<aligned_pointer_type>(end_iterator.group_pointer->skipfield) -
                                            end_iterator.element_pointer);
            ...
            {                         // Mark unused element memory locations from back group as skipped/erased:
                ...
                if (previous_node_value == 0) // no previous skipblock
                {
                    ...
                    *(end_iterator.skipfield_pointer + distance_to_end - 1) = distance_to_end;
                    ...
                    {
                        edit_free_list_next(
                            end_iterator.group_pointer->elements + end_iterator.group_pointer->free_list_head,
                            index); // set prev free list head's 'next index' number to the index of the current element
                    } else {
        ...
        if (original_unused_groups_head != NULL) // If there were unused groups in source, re-link them and remove their
                                                 // capacity count from *this while adding it to source:
        {
            ...
            // Establish first group from source unused_groups as first active group in source, link rest as reserved
            // groups:
        ...
        if (number_of_elements_needed < max_block_capacity) {
            ...
            for (group_pointer_type current = unused_groups_head; current != NULL; current = current->next_group) {
                if (current->capacity >= number_of_elements_needed)
                    return current->elements; // ie. there is enough space in one of the unused blocks
            }
        }
...
public:
    ...
    void sort(comparison_function compare) {
        ...
        if constexpr ((std::is_trivially_copyable<element_type>::value ||
                       std::is_move_assignable<element_type>::value) &&
                      sizeof(element_type) <=
                          sizeof(element_type *) * 2) // If element is <= 2 pointers, just copy to an array and sort
                                                      // that then copy back - consumes less memory
        {
            ...
            if constexpr (!std::is_trivially_copy_constructible<element_type>::value &&
                          std::is_nothrow_move_assignable<element_type>::value) {
                std::uninitialized_copy(plf::make_move_iterator(begin_iterator), plf::make_move_iterator(end_iterator),
                                        sort_array);
            } else {
            ...
            if (need_to_allocate) {
                sort_array =
                    PLF_ALLOCATE(tuple_allocator_type, tuple_allocator, total_size, end_iterator.skipfield_pointer);
            }
    ...
    void swap(colony &source) PLF_NOEXCEPT_SWAP(allocator_type) {
        ...
        if constexpr (std::allocator_traits<allocator_type>::is_always_equal::value &&
                      ...
                      std::is_trivially_copyable<
                          skipfield_pointer_type>::value) // if all pointer types are trivial we can just copy using
                                                          // memcpy - avoids constructors/destructors etc and is faster
        {
            ...
            *this = std::move(temp);
            ...
            // Otherwise, make the reads/writes as contiguous in memory as-possible (yes, it is faster than using
            // std::swap with the individual variables):
    ...
    struct colony_data : public uchar_allocator_type {
        ...
        unsigned char **const bitfield_pointers; // array of pointers to bitfields in the form of unsigned char arrays
        ...
        colony_data(const colony::size_type size)
            : block_pointers(pointer_cast<aligned_pointer_type *>(
              ...
              block_capacities(
                  pointer_cast<size_t *>(PLF_ALLOCATE(uchar_allocator_type, *this, size * sizeof(size_t), NULL))),
...
public:
    colony_data *data() {
        ...
        for (group_pointer_type current_group = begin_iterator.group_pointer;
             current_group != end_iterator.group_pointer; current_group = current_group->next_group, ++group_number) {
        ...
        setup_data_cell(data, end_iterator.group_pointer,
                        static_cast<size_t>(end_iterator.element_pointer - end_iterator.group_pointer->elements),
                        group_number);
    ...
    public:
        ...
        typedef
            typename plf::conditional<is_const, typename colony::const_pointer, typename colony::pointer>::type pointer;
        typedef typename plf::conditional<is_const, typename colony::const_reference, typename colony::reference>::type
            reference;
        ...
        colony_iterator() noexcept : group_pointer(NULL), element_pointer(NULL), skipfield_pointer(NULL) {}

        colony_iterator(const colony_iterator &source) noexcept
            : // Note: Surprisingly, use of = default here and in other simple constructors results in slowdowns of up
              // to 10% in many benchmarks under GCC
        ...
        colony_iterator(const colony_iterator<false> &source) noexcept
            : group_pointer(source.group_pointer), element_pointer(source.element_pointer),
              skipfield_pointer(source.skipfield_pointer) {}
        ...
        template <bool is_const_it = is_const, class = typename plf::enable_if<is_const_it>::type>
        colony_iterator(colony_iterator<false> &&source) noexcept
            : group_pointer(std::move(source.group_pointer)), element_pointer(std::move(source.element_pointer)),
              skipfield_pointer(std::move(source.skipfield_pointer)) {}

        colony_iterator &operator=(const colony_iterator &source) noexcept {
        ...
        template <bool is_const_it = is_const, class = typename plf::enable_if<is_const_it>::type>
        colony_iterator &operator=(const colony_iterator<false> &source) noexcept {
            group_pointer = source.group_pointer;
            element_pointer = source.element_pointer;
            skipfield_pointer = source.skipfield_pointer;
            return *this;
        ...
        colony_iterator &operator=(colony_iterator &&source) noexcept {
            assert(&source != this);
        ...
        template <bool is_const_it = is_const, class = typename plf::enable_if<is_const_it>::type>
        colony_iterator &operator=(colony_iterator<false> &&source) noexcept
        ...
        bool operator==(const colony_iterator &rh) const noexcept { return (element_pointer == rh.element_pointer); }

        bool operator==(const colony_iterator<!is_const> &rh) const noexcept {
        ...
        colony_iterator &operator++() {
            ...
            if ((element_pointer += static_cast<size_type>(skip) + 1u) ==
                ...
                group_pointer->next_group != NULL) // ie. beyond end of current memory block. Second condition allows
                                                   // iterator to reach end(), which may be 1 past end of block, if
                                                   // block has been fully used and another block is not allocated
            {
        ...
        colony_iterator &operator--() {
            ...
            if (--skipfield_pointer >=
                group_pointer->skipfield) // ie. not already at beginning of group prior to decrementation
            {
        ...
        bool operator>(const colony_iterator<is_const_it> &rh) const noexcept {
    ...
    private:
        ...
        void advance(difference_type distance) // Cannot be noexcept due to the possibility of an uninitialized iterator
        {
            ...
            {
                // Code explanation:
                // For the initial state of the iterator, we don't know which elements have been erased before that
                // element in that group. So for the first group, we follow the following logic:
                // 1. If no elements have been erased in the group, we do simple pointer addition to progress, either to
                // within the group (if the distance is small enough) or the end of the group and subtract from distance
                // accordingly.
                // 2. If any of the first group's elements have been erased, we manually iterate, as we don't know
                // whether the erased elements occur before or after the initial iterator position, and we subtract 1
                // from the distance amount each time we iterate. Iteration continues until either distance becomes
                // zero, or we reach the end of the group.

                // For all subsequent groups, we follow this logic:
                // 1. If distance is larger than the total number of non-erased elements in a group, we skip that group
                // and subtract the number of elements in that group from distance.
                // 2. If distance is smaller than the total number of non-erased elements in a group, then:
                //   a. If there are no erased elements in the group we simply add distance to group->elements to find
                //   the new location for the iterator. b. If there are erased elements in the group, we manually
                ...
                // Special case for initial element pointer and initial group (we don't know how far into the group the
                // element pointer is)
                if (element_pointer !=
                    group_pointer->elements +
                        *(group_pointer
                              ->skipfield)) // ie. != first non-erased element in group - otherwise we skip this section
                                            // and just treat the first block as we would an intermediary block
                {
                        ...
                        std::numeric_limits<skipfield_type>::max()) // ie. if there are no erasures in the group
                    {
                        if (distance < distance_from_end) {
                            ...
                            return;
                                   ...
                                   NULL) // either we've reached end() or gone beyond it, so bound to back of block
                        {
                        ...
                        if (group_pointer->next_group ==
                            NULL) // either we've reached end() or gone beyond it, so bound to end of block
                        {
                            element_pointer = pointer_cast<aligned_pointer_type>(group_pointer->skipfield);
                            return;
                        }
                ...
                // Intermediary groups - at the start of this code block and the subsequent block, the position of the
                // iterator is assumed to be the first non-erased element in the current group:
                ...
                if (group_pointer->free_list_head ==
                    std::numeric_limits<skipfield_type>::max()) // No erasures in this group, use straight pointer
                                                                // addition
                {
                    element_pointer = group_pointer->elements + distance;
                    skipfield_pointer = group_pointer->skipfield + distance;
                } else // We already know size > distance due to the intermediary group checks above - safe to ignore
                ...
                // Special case for initial element pointer and initial group (we don't know how far into the group the
                // element pointer is)
                if (element_pointer != pointer_cast<aligned_pointer_type>(
                                           group_pointer->skipfield)) // not currently at the back of a block
                {
                        ...
                        std::numeric_limits<skipfield_type>::max()) // ie. no prior erasures have occurred in this group
                    {
                        ...
                        if (group_pointer->previous_group == NULL) {
                            element_pointer =
                                group_pointer->elements +
                                *(group_pointer->skipfield); // This is first group, so bound to begin() (just in case
                                                             // final decrement took us before begin())
                            skipfield_pointer = group_pointer->skipfield + *(group_pointer->skipfield);
                            return;
                        }
                    }
                ...
                // Intermediary groups - at the start of this code block and the subsequent block, the position of the
                // iterator is assumed to be either the first non-erased element in the next group over, or end():
                while (static_cast<difference_type>(group_pointer->size) < distance) {
                ...
                } else // ie. no more groups to traverse but there are erased elements in this group
                {
                    skipfield_pointer =
                        ...
                        (pointer_cast<aligned_pointer_type>(group_pointer->skipfield) - group_pointer->elements);
        ...
        difference_type distance(const colony_iterator &last) const {
            ...
            // If they are not pointing to elements in the same group, process the intermediate groups and add
            // distances, skipping manual incrementation in all but the initial and final groups. In the initial and
            // final groups, manual incrementation must be used to calculate distance, if there have been no prior
            // erasures in those groups. If there are no prior erasures in either of those groups, we can use pointer
            ...
            if (iterator1.group_pointer != iterator2.group_pointer) // if not in same group, process intermediate groups
            {
                ...
                if (iterator1.group_pointer->free_list_head ==
                    std::numeric_limits<skipfield_type>::max()) // If no prior erasures have occured in this group we
                    ...
                    distance += static_cast<difference_type>(
                        ...
                        iterator1.element_pointer);
                           ...
                           iterator1.group_pointer->elements +
                               *(iterator1.group_pointer->skipfield)) // ie. element is at start of group - rare case
                {
            ...
            if (iterator2.group_pointer->free_list_head ==
                std::numeric_limits<skipfield_type>::max()) // ie. no erasures in this group, direct subtraction is
                                                            // possible
            {
                distance += iterator2.skipfield_pointer - iterator1.skipfield_pointer;
            } else if (iterator1.element_pointer ==
                           iterator2.group_pointer->elements + *(iterator2.group_pointer->skipfield) &&
                       iterator2.element_pointer + 1 + *(iterator2.skipfield_pointer + 1) ==
                           pointer_cast<aligned_pointer_type>(
                               iterator2.group_pointer
                                   ->skipfield)) // ie. if iterator1 is at beginning of block (have to check this in
                                                 // case first and last are in the same block to begin with) and
                                                 // iterator2 is last element in the block
            {
    ...
    public:
        ...
        typedef typename colony::value_type value_type;
        typedef typename colony::difference_type difference_type;
        typedef typename plf::conditional<is_const_r, typename colony::const_pointer, typename colony::pointer>::type
            pointer;
        typedef
            typename plf::conditional<is_const_r, typename colony::const_reference, typename colony::reference>::type
                reference;
        ...
        colony_reverse_iterator(const colony_reverse_iterator &source) noexcept : current(source.current) {}

        template <bool is_const_rit = is_const_r, class = typename plf::enable_if<is_const_rit>::type>
        colony_reverse_iterator(const colony_reverse_iterator<false> &source) noexcept : current(source.current) {}

        colony_reverse_iterator(const colony_iterator<is_const_r> &source) noexcept : current(source) { ++(*this); }

        template <bool is_const_rit = is_const_r, class = typename plf::enable_if<is_const_rit>::type>
        colony_reverse_iterator(const colony_iterator<false> &source) noexcept
            :
        ...
        colony_reverse_iterator(colony_reverse_iterator &&source) noexcept : current(std::move(source.current)) {}

        template <bool is_const_rit = is_const_r, class = typename plf::enable_if<is_const_rit>::type>
        colony_reverse_iterator(colony_reverse_iterator<false> &&source) noexcept
            :

              current(std::move(source.current)) {}

        colony_reverse_iterator &operator=(const colony_iterator<is_const_r> &source) noexcept {
            current = source;
            ++current;
            return *this;
        }
        ...
        template <bool is_const_rit = is_const_r, class = typename plf::enable_if<is_const_rit>::type>
        colony_reverse_iterator &operator=(const colony_iterator<false> &source) noexcept
        ...
        colony_reverse_iterator &operator=(const colony_reverse_iterator &source) noexcept {
            current = source.current;
            return *this;
        }

        template <bool is_const_rit = is_const_r, class = typename plf::enable_if<is_const_rit>::type>
        colony_reverse_iterator &operator=(const colony_reverse_iterator<false> &source) noexcept
        ...
        colony_reverse_iterator &operator=(colony_reverse_iterator &&source) noexcept {
            assert(&source != this);
            current = std::move(source.current);
            return *this;
        }
        ...
        template <bool is_const_rit = is_const_r, class = typename plf::enable_if<is_const_rit>::type>
        colony_reverse_iterator &operator=(colony_reverse_iterator<false> &&source) noexcept {
            assert(&source != this);
            current = std::move(source.current);
            return *this;
        }

        bool operator==(const colony_reverse_iterator &rh) const noexcept { return (current == rh.current); }

        bool operator==(const colony_reverse_iterator<!is_const_r> &rh) const noexcept {
        ...
        reference operator*() const noexcept { return *pointer_cast<pointer>(current.element_pointer); }

        pointer operator->() const noexcept { return pointer_cast<pointer>(current.element_pointer); }
        ...
        colony_iterator<is_const_r> base() const noexcept {
            return (current.group_pointer != NULL) ? ++(colony_iterator<is_const_r>(current))
                                                   : colony_iterator<is_const_r>(NULL, NULL, NULL);
        }

        template <bool is_const_it>
        bool operator>(const colony_reverse_iterator<is_const_it> &rh) const noexcept {
    ...
    private:
        ...
        // Advance for reverse_iterator and const_reverse_iterator - this needs to be implemented slightly differently
        // to forward-iterator's advance, as current needs to be able to reach rend() (ie. begin() - 1) and to be
        // bounded by rbegin():
        void advance(difference_type distance) {
            ...
            if (distance > 0) {
                if (group_pointer->previous_group == NULL && element_pointer == group_pointer->elements - 1)
                ...
                // Intermediary groups - at the start of this code block and the subsequent block, the position of the
                // iterator is assumed to be the first non-erased element in the next group:
                while (static_cast<difference_type>(group_pointer->size) < distance) {
                ...
                while (static_cast<difference_type>(group_pointer->size) <= distance) {
                    if (group_pointer->next_group == NULL) // bound to last element slot in block
                    {
                        ...
                        element_pointer =
                            (pointer_cast<aligned_pointer_type>(group_pointer->skipfield) - 1) - *skipfield_pointer;
                        skipfield_pointer -= *skipfield_pointer;
                        return;
                    } else if ((distance -= group_pointer->size) == 0) {
                        group_pointer = group_pointer->next_group;
                ...
                if (group_pointer->free_list_head == std::numeric_limits<skipfield_type>::max()) {
                    ...
                    skipfield_pointer = group_pointer->skipfield + distance;
                    return;
                } else // we already know size > distance from previous loop - so it's safe to ignore endpoint check
                       // condition while incrementing:
                {
                    skipfield_pointer = group_pointer->skipfield + *(group_pointer->skipfield);
...
void swap(plf::colony<element_type, allocator_type> &a, plf::colony<element_type, allocator_type> &b)
    PLF_NOEXCEPT_SWAP(allocator_type) {
    a.swap(b);
}
