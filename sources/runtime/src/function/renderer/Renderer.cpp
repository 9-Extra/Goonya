...
void Renderer::render() {
    ...
    for (auto &&rt : replace_render_target) {
        if (!rt || rt->get_size() != std::make_tuple(w, h)) {
            ...
            rt->set_color_renderbuffer(0, TextureStorageFormat::RGB_f8);
    ...
    for (auto &&camera : camera_set) {
        if (!camera->render_target || !scene_set.contains(camera->scene)) continue;
        ...
        for (auto &&s : scene_set[camera->scene].skyboxs) {
